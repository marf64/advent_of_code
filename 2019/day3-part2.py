...
def lay_straight_path(start, direction, distance, points, value, step_counter, steps):
	loc = start
	for d in range(distance):
		steps += 1
		loc = vec_add(start, vec_mul(d+1, direction))
		p = points.get(loc, 1)
		points[loc] = p*value
		step_counter[loc] = steps
	return (points, loc, step_counter, steps)


def get_direction(d_str):
	d_str = d_str[0]
	if d_str == 'R':
		return (1, 0)
	elif d_str == 'L':
...
def lay_wire(wire, points, value, step_counter):
	loc = (0,0)
	steps = 0
	for d in wire:
		direction = get_direction(d)
		distance = int(d[1:])
		points, loc, step_counter, steps = lay_straight_path(loc, direction, distance, points, value, step_counter, steps)

	return points

def distance(point):
	p1 = point[0] if point[0] >= 0 else -point[0]
	p2 = point[1] if point[1] >= 0 else -point[1]
	return p1+p2

steps1 = dict()
steps2 = dict()

points = lay_wire(wire1, points, 2, steps1)
points = lay_wire(wire2, points, 3, steps2)



# m = min(distance(p[0]) for p in points.items() if p[0] != (0, 0) and p[1] % 2 == 0 and p[1] %3 == 0)
# m = [distance(p[0]) for p in points.items() if p[0] != (0, 0) and p[1] % 2 == 0 and p[1] %3 == 0]
# print(m)

ints = [p[0] for p in points.items() if p[0] != (0, 0) and p[1] % 2 == 0 and p[1] % 3 == 0]
m = min(steps1[p] + steps2[p] for p in ints)
print(m)